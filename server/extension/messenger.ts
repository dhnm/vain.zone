...
const getPlayerInfo = (params: {
  ...
  getData({ IGN: params.IGN, playerID: params.playerID, messenger: true })
    .then((data: any) => {
    ...
    .then(user => {
      if (user.defaultIGN) {
        ...
        const messageData = {
          ...
          message: {
            text:
              'You can set your default nick by sending it here and clicking on "Save" button that comes afterwards.'
          }
        };
...
const sendPlayerInfo = (userID: string, player: any) => {
  let rankPointsText = "";

  if (player.rank_5v5) {
    rankPointsText = `3v3 / 5v5 Rank Points: ${Math.floor(
      player.rank_3v3
    )} / ${Math.floor(player.rank_5v5)}`;
  } else {
  ...
  let tier = "";
  if (player.rank_5v5) {
    const processed3v3 = skillTierCalculator(player.rank_3v3);
    const processed5v5 = skillTierCalculator(player.rank_5v5);
    tier = `(${processed3v3.number}${processed3v3.shortColor}/${
      processed5v5.number
    }${processed5v5.shortColor})`;
  } else {
    const processed3v3 = skillTierCalculator(player.rank_3v3);
    tier = `(${processed3v3.number}${processed3v3.shortColor})`;
  }
                ...
                buttons: [
                  {
                    title: "Save",
                    type: "postback",
                    payload: `set ${player.name},,,${player.playerID},,,${
                      player.shardId
                    }`
                  }
                ]
...
const setIGN = (instructions: string, userID: string) => {
  findOrCreateUser(userID)
    .then(user => {
