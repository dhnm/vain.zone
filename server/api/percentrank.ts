...
router.get("/en/:IGN", cacheMW(3600), (req, res) => {
  Player.findOne({ name: req.params.IGN })
    ...
    .catch(() => {
      axios(
        `https://api.vgpro.gg/player/${req.params.IGN}/stats?season=summer18`
      )
        .then(axiosResponse => {
...
router.get("/:IGN", cacheMW(3600), (req, res): void => {
  Player.findOne({ name: req.params.IGN })
    .exec()
    .then((player: IPlayer) => {
      if (player) {
