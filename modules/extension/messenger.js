...
const receivedMessage = event => {
    ...
    if (messageText) {
        ...
        if (msgArray.length == 1 && msgArray[0].toLowerCase() == "help") {
            ...
            msgArray.length == 1 &&
            (msgArray[0].toLowerCase() == "league" ||
                msgArray[0].toLowerCase() == "hyacinth")
        ) {
            sendHyacinth(userID);
        } else if (
            msgArray.length == 1 &&
            (msgArray[0].toLowerCase() == "inae" ||
                msgArray[0].toLowerCase() == "inaequalis")
        ) {
            getTrainingURL(userID);
...
const getPlayerInfo = (input, userID) => {
    ...
    getData(req.query.IGN)
        .then(data => {
            ...
            playerData.ranked = player.gamesPlayed.ranked;
            playerData.blitz = player.played_blitz;

            var tier = parseInt(player.skillTier / 3) + 1;
            var color = player.skillTier % 3;
            switch (color) {
                case 0:
                    color = "B";
                    break;
            ...
            sendPlayerInfo(senderID, playerData);
        })
        .catch(err => {
            console.log(err);
            sendSystemMessage(
...
const findOrCreateUser = psid => {
    return BotUser.findOne({ psid: recipientID })
        .exec()
        .then(user => {
            if (user) {
                return Promise.resolve(user);
            } else {
                const newUser = new BotUser({ psid: recipientID });
                return newUser
                    .save()
                    .then(savedUser => {
    ...
    findOrCreateUser(userID)
        ...
        .catch(err => {
            var messageData = {
                ...
                message: {
                    text:
                        "An error has occured, please contact the devlopers. " +
                        err
                }
            };
                ...
                payload: {
                    ...
                    elements: [
                        ...
                        },
                        {
                            ...
                            subtitle:
                                "Rank points: " +
                                parseInt(data.rankPoints.ranked) +
                                "\nTotal wins: " +
                                data.wins
                        }
                    ],
                    buttons: [
                        {
                            type: "web_url",
                            url:
                                "https://test.vainglory.eu/extension/player/" +
                                data.name,
                            title: "See more",
    ...
    findOrCreateUser(userID)
        ...
        .catch(err => {
            var messageData = {
                ...
                message: {
                    text:
                        "An error has occured, please contact the developers. " +
                        err
                }
            };
