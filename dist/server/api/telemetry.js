    ...
    return axios_1.default({
        ...
        for (let rosterIndex = 0; rosterIndex < matchData.rosters.length; rosterIndex++) {
            ...
            for (let participantIndex = 0; participantIndex < currentRoster.participants.length; participantIndex++) {
                ...
                const player = match.included.find((e) => e.id === currentParticipant.player.playerID);
...
const loopThroughTelemetry = (telemetryData, matchData) => {
    ...
    let roleDetection = false;
    if (matchData.rosters[0].participants.length +
        matchData.rosters[1].participants.length ===
        10 &&
        matchData.duration > 540) {
        roleDetection = "5v5";
    }
    else if (matchData.rosters[0].participants.length +
        matchData.rosters[1].participants.length ===
        6 &&
        ["ranked", "private_party_draft_match", "casual", "private"].indexOf(matchData.gameMode) > -1 &&
    ...
    const creepKills = [{}, {}];
    const matchTimerStartTime = moment(telemetryData.find(e => e.type === "PlayerFirstSpawn").time);
    const first5v5TurretKillObject = roleDetection === "5v5"
        ? telemetryData.find(e => e.type === "KillActor" && e.payload.Killed === "*OuterTurret5v5*") ||
            telemetryData.find(e => e.type === "KillActor" && e.payload.Killed === "*Turret5v5*")
        : undefined;
    const first5v5TurretKillTime = roleDetection === "5v5" ? moment(first5v5TurretKillObject.time) : undefined;
    const draftOrder = [[], []];
    for (let i = 0; i < telemetryData.length; i++) {
        ...
        if (currVa.type === "DealDamage" && currVa.payload.TargetIsHero == 1) {
            ...
            moment(currVa.time).unix() - matchTimerStartTime.unix() <=
                Math.min(390, Math.max(270, first5v5TurretKillTime.unix() - matchTimerStartTime.unix())) &&
            currVa.type === "KillActor" &&
            ...
            const actorName = currVa.payload.Actor.replace(/\*/g, "");
            if (reference[actorName]) {
                const minionType = detectMinionType({ Left: 0, Right: 1 }[currVa.payload.Team], currVa.payload.Position, currVa.payload.KilledTeam === "Neutral");
                if (minionType) {
            ...
            creepKills[{ Left: 0, Right: 1 }[currVa.payload.Team]][heroName] = constants_1.talentsDictionary[heroName]
                ? {
                    rarity: constants_1.talentsDictionary[heroName][currVa.payload.Talent],
...
const getRoles = (roleDetection, matchData, creepKills) => {
    ...
    if (roleDetection === "3v3") {
        ...
        unsureRoles.forEach(side => side.forEach(u => {
            const missingJungler = filledRoles[u.sideIndex].indexOf("2:jungler") === -1;
    ...
    if (roleDetection === "5v5") {
        ...
        creepKills.forEach((side, sideIndex) => {
            Object.keys(side).forEach(actor => {
                for (let role in creepKillMaxValues[0]) {
                    ...
                    if (actorRolesData[role] >
                        Math.max(...Object.keys(actorRolesData)
                            .filter(e => e !== role)
        ...
        matchData.rosters.forEach((side, rosterIndex) => side.participants.forEach(p => {
