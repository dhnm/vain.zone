                    ...
                    .then(newDBPlayerData => {
                        if (newDBPlayerData) {
                            if (newDBPlayerData.name) {
                                newDBPlayerData.IGNHistory.push(
                                    newDBPlayerData.name
                                );
                            }
...
function getPlayerFromAPI(params: {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (params.shardId) {
            regions.unshift(
                regions.splice(regions.indexOf(params.shardId), 1)[0]
            );
        }
                ...
                .then(json => {
                    ...
                    try {
                        customPlayerDataModel = {
                            ...
                            played_blitz:
                                data.attributes.stats.gamesPlayed.blitz,
                            played_casual:
                                data.attributes.stats.gamesPlayed.casual,
                            played_ranked:
                                data.attributes.stats.gamesPlayed.ranked,
                            played_casual_5v5:
                            ...
                            rank_5v5:
                                data.attributes.stats.rankPoints.ranked_5v5,
    ...
    return axios({
        ...
        .catch(error => {
            if (error.response) {
                ...
                console.error(
                    "axiosAPI: No response from the API.",
                    error.request
                );
